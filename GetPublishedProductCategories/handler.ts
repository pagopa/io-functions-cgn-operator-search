...
export const GetPublishedProductCategoriesHandler = (
  ...
  pipe(
    ...
    TE.map(
      flow(
        AR.map(productCategoryModel => ProductCategoryFromModel(productCategoryModel.product_category)),
        productCategories => ({ items: productCategories })
      )
    ),
    TE.chain(
      flow(PublishedProductCategories.decode, TE.fromEither, TE.mapLeft(errorsToError))
    ),
